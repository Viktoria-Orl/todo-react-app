...
function WeekProgressWidget({list, today}: WeekProgressWidgetProps) {
    ...
    function isCompletedCell(listItem: TList, days: number) : string {
        const [ date ] = new Date(today.getTime() - days * (24 * 60 * 60 * 1000)).toISOString().split('T');
        return listItem.completedDates.includes(date) ? '✅' : '❌';
    }

    return <div className="weekProgressWidget">
        <table>
            <thead>
                <tr>
                    <th></th>
            ...
            <tbody>
                {list.filter(listItem => !listItem.isDeleted).map(listItem => {
